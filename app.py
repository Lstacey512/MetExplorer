...
def SearchArtist():
    ...
    if Artist_name != "None":
        ...
        try: 
            id_Lookup = session.query(Artists.Artist_id).filter(Artists.Full_name.ilike('%'+Artist_name+'%')).first()
            Artist_id = id_Lookup[0]
            print(Artist_id, file=sys.stderr)
        except:
            Artist_id = 2
    ...
    #if we have an id arguement and it's not assocaited with an "unknown author"
    if Artist_id == 2 or Artist_id == 320 or Artist_id == 684:
        return render_template ("404.html", error = "We couldn't seem to find the artist. They might be unknown or don't exist in the Met's public domain collection. Try again by refining your search on the homepage or by selecting a painting via search.")
